...
class GraphicBoard(ParamInputMixin):
    ...
    SUITS = {
        "golds": {"symbol": "🪙", "color": SUIT_COLORS["golds"]},
    ...
    def ask_player_count(self) -> int:
        ...
        while self.running:
            ...
            self._draw_text("Press 2, 3, or 4 to select number of players:", 
                          self.font_large, self.white, 50, 200)
            pygame.display.flip()
            self.clock.tick(60)
    ...
    def ask_player_names(self, count: int) -> list[str]:
        ...
        for i in range(count):
            while self.running:
                ...
                self.screen.fill(self.bg_color)
                self._draw_text(f"Enter name for player {i+1}:", 
                              self.font_large, self.white, 50, 200)
                self._draw_text(current_name, self.font_medium, self.white, 50, 250)
                self._draw_text("Press Enter to confirm, Q to quit", 
                              self.font_small, self.gray, 50, 300)
                pygame.display.flip()
                self.clock.tick(60)
    ...
    def ask_race_length(self) -> int:
        ...
        while self.running:
            ...
            self.screen.fill(self.bg_color)
            self._draw_text("Press 4, 5, 6, or 7 to select race length:", 
                          self.font_large, self.white, 50, 200)
            pygame.display.flip()
            self.clock.tick(60)
    ...
    def draw_game(self, game: Game):
        ...
        # Draw instructions
        self._draw_text("Press Q to quit, any other key to continue", 
                       self.font_small, self.gray, 50, self.height - 30)
    ...
    def _draw_race_track(self, game: Game):
        ...
        pygame.draw.line(self.screen, self.white, 
                        ...
                        (finish_x, track_start_y + game.players * self.CARD_HEIGHT), 3)
        self._draw_text("FINISH", self.font_medium, self.white, finish_x + 10, track_start_y)
    ...
    def _draw_card(self, x: int, y: int, value, suit: Optional[str]):
        ...
        if suit:
            ...
            text_rect = text_surface.get_rect(center=(x + self.CARD_WIDTH//2, y + self.CARD_HEIGHT//2 - 10))
            ...
            suit_rect = suit_surface.get_rect(center=(x + self.CARD_WIDTH//2, y + self.CARD_HEIGHT//2 + 15))
    ...
    def _draw_text(self, text: str, font: pygame.font.Font, color: tuple, x: int, y: int):
    ...
    def _ask_yes_no(self, question: str) -> bool:
        ...
        while self.running:
            ...
            self._draw_text("Press S for Yes, N for No", self.font_medium, self.gray, 50, 250)
            pygame.display.flip()
            self.clock.tick(60)
